            ...
            <div>
                ...
                <div x-show="viewMode === 'gantt'" wire:key="gantt-view-{{ $selectedPlan }}">
                    ...
                    <div class="flex justify-end mb-4 px-1 gap-2">
                        @if ($selectedPlan)
                            <a href="{{ route('projects.plans.export-gantt', ['project' => $projectId, 'plan' => $selectedPlan]) }}"
                                target="_blank"
...
@push('scripts-content')
    <script>     (function () {
            if (typeof Sortable === 'undefined') { console.error('❌ SortableJS no está cargado.'); return; }
            ...
            window.initTaskSorting = function (container, $wire) {
                ...
                new Sortable(container, {
                    group: 'tasks', animation: 150, forceFallback: true, fallbackOnBody: true, swapThreshold: 0.65, onEnd(evt) {
                        const oldBucketId = evt.from?.dataset?.bucketId; const newBucketId = evt.to?.dataset?.bucketId; const orderedIds = Array.from(evt.to.children).map(el => el.dataset?.id).filter(Boolean);
