<x-dialog-modal wire:model="showModal" position="center">
    ...
    <x-slot name="content">
        <div x-data="{ tab: 'users' }" class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    @else
                        <div class="bg-gray-100 p-2 rounded-md flex justify-between items-center">
                            <input type="text" readonly value="{{ $shareLink }}" class="bg-transparent w-full text-sm" />
    ...
    <x-slot name="footer">
        ...
        <x-button wire:click="save" class="bg-yellow-700 hover:bg-yellow-800 text-white ml-2" x-show="tab !== 'link'"
            x-bind:disabled="(tab === 'users' && @js($selectedUserId) === null) || (tab === 'teams' && @js($selectedTeamId) === null)">
            Compartir
        </x-button>
    </x-slot>
</x-dialog-modal>