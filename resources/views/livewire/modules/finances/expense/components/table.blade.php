    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 gap-4">
        ...
        <div class="flex items-center gap-2 w-full md:w-auto">
            <div class="relative w-full md:w-64">
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="Buscar gastos..."
            ...
            <x-button href="{{ route('finances.expenses.create') }}" wire:navigate>Nuevo Gasto</x-button>
        </div>
    </div>
            ...
            <tbody class="text-gray-800">
                @forelse($expenses as $expense)
                    <tr class="hover:bg-gray-50">
                        <td class="p-2">{{ $expense->name }} @if($expense->description) <span
                        class="block text-xs text-gray-500">{{ $expense->description }}</span> @endif</td>
                        <td class="p-2 text-center">{{ $expense->date }}</td>
                        <td class="p-2 text-center text-red-600 font-semibold">
                            {{ App\Services\MoneyFormatterService::format($expense->amount) }}
                        </td>
                        <td class="p-2 text-center"><span
                                class="bg-gray-100  text-xs px-2 py-1 rounded">{{ $expense->result->name }}</span></td>
                        <td class="p-2 text-center">
                            <a href="{{ route('finances.expenses.edit', $expense) }}" wire:navigate
