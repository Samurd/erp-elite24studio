...
const submit = () => {
    if (isEditing.value) {
        form.post(route('finances.invoices.clients.billing-accounts.update', props.invoice.id), {
            forceFormData: true,
        })
    } else {
        form.post(route('finances.invoices.clients.billing-accounts.store'), {
            forceFormData: true,
        })
    }
}
...
<template>
    <AppLayout :title="isEditing ? 'Editar Cuenta de Cobro' : 'Crear Cuenta de Cobro'">
        <div class="max-w-4xl mx-auto p-6">
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex justify-between items-center border-b pb-4 mb-6">
                    <h2 class="text-2xl font-bold text-gray-800">
                        {{ isEditing ? 'Editar Cuenta de Cobro' : 'Crear Cuenta de Cobro' }}
                    </h2>
                </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows="3"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                            placeholder="Descripción de la cuenta de cobro..."
                        ></textarea>
                    ...
                    <div class="flex justify-end gap-3 border-t pt-6">
                        <Link
                            :href="route('finances.invoices.clients.billing-accounts.index')"
