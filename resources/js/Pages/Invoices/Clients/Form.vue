...
const submit = () => {
    if (isEditing.value) {
        form.post(route('finances.invoices.clients.update', props.invoice.id), {
            forceFormData: true,
        })
    } else {
        form.post(route('finances.invoices.clients.store'), {
            forceFormData: true,
        })
    }
}
...
<template>
    <AppLayout :title="isEditing ? 'Editar Factura DIAN' : 'Crear Factura DIAN'">
        <div class="max-w-4xl mx-auto p-6">
            <div class="bg-white rounded-lg shadow-sm p-6">
                <div class="flex justify-between items-center border-b pb-4 mb-6">
                    <h2 class="text-2xl font-bold text-gray-800">
                        {{ isEditing ? 'Editar Factura DIAN' : 'Crear Factura DIAN' }}
                    </h2>
                </div>
