...
const openNewRoleModal = async () => {
    await fetchPermissions(); // Ensure permissions are loaded
...
const openEditRoleModal = async () => {
    ...
    try {
        await fetchPermissions(); // Ensure permissions are loaded first
        
        const response = await axios.get(route('users.roles.get', form.role));
    ...
    <AppLayout :title="isEdit ? 'Editar Usuario' : 'Crear Usuario'">
        ...
        <DialogModal :show="openCreateRoleModal" @close="openCreateRoleModal = false">
            ...
            <template #footer>
                ...
                <button class="ml-2 inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150" @click="saveRole">
