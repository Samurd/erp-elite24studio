...
const projectTypeFilter = ref(props.filters.project_type_filter || '')
const contactFilter = ref(props.filters.contact_filter || '')
const responsibleFilter = ref(props.filters.responsible_filter || '')
const currentStageFilter = ref(props.filters.current_stage_filter || '')
...
const applyFilters = () => {
    router.get(route('projects.index'), {
        ...
        project_type_filter: projectTypeFilter.value,
        contact_filter: contactFilter.value,
        responsible_filter: responsibleFilter.value,
        current_stage_filter: currentStageFilter.value,
    ...
    <AppLayout title="Proyectos">
        <div class="p-6">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Gestión completa de proyectos</p>
                    </div>
                    <Link
            ...
            <div class="bg-white rounded-lg shadow-sm overflow-hidden">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Etapa Actual</th>
                            ...
                            <tr v-for="project in projects.data" :key="project.id" class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-gray-500 mt-1">
                                        ...
                                        <span v-if="project.direction" class="ml-1">• {{ project.direction }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ project.contact?.name || '-' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span v-if="project.status" :class="['px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full', getStatusColor(project.status.name)]">
                                        {{ project.status.name }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span v-if="project.current_stage" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-purple-100 text-purple-800">
                                        {{ project.current_stage.name }}
                                    </span>
                                    <span v-else class="text-gray-400 text-sm">-</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center" v-if="project.responsible">
                                        ...
                                        <span class="text-sm text-gray-900">{{ project.responsible.name }}</span>
                                    </div>
                                    <span v-else class="text-gray-400 text-sm">-</span>
                                </td>
                ...
                <!-- Pagination -->
                <div v-if="projects.links && projects.data.length > 0" class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Mostrando <span class="font-medium">{{ projects.from }}</span> a
                                <span class="font-medium">{{ projects.to }}</span> de
                                <span class="font-medium">{{ projects.total }}</span> resultados
                            </p>
                        </div>
