...
const form = useForm({
    ...
    created_by_id: props.expense?.created_by_id || '',
    result_id: props.expense?.result_id || '',
})
...
const submit = () => {
    if (isEditing.value) {
        form.put(route('finances.expenses.update', props.expense.id))
    } else {
        form.post(route('finances.expenses.store'))
    }
}
</script>

<template>
    <AppLayout :title="isEditing ? 'Actualizar Gasto' : 'Añadir nuevo Gasto'">
        <div class="max-w-4xl mx-auto p-4">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <div class="flex border-b pb-4 mb-4">
                    <h3 class="text-xl font-bold">{{ isEditing ? 'Actualizar Gasto' : 'Añadir nuevo Gasto' }}</h3>
                </div>
                ...
                <form @submit.prevent="submit" class="flex flex-col gap-4">
                    ...
                    <div class="flex justify-end mt-6 pt-4 border-t gap-2">
                        <Link
                            :href="route('finances.expenses.index')"
