...
namespace App\Livewire\Forms\Modules\Finances\Invoices\Providers;
...
class Form extends LivewireForm
{
    ...
    {
        // Generate unique alphanumeric code like "INV-PRV-20251124-ABCD"
        $date = date('Ymd');
        $random = strtoupper(substr(md5(uniqid(rand(), true)), 0, 4));

        $code = "INV-PRV-{$date}-{$random}";
        ...
        while (Invoice::where('code', $code)->exists()) {
            ...
            if ($counter > 100) {
                ...
                $code = "INV-PRV-{$date}-" . strtoupper(substr(md5(microtime()), 0, 6));
                break;
            }
        }
